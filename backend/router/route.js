...
router.post('/tasks', async (req, res) => {
  const { title, boardId, listId, description, position, userId } = req.body;
...
router.post('/tasks/:taskId/reminder', async (req, res) => {
  ...
  task.reminder = true;
  await task.save();
  await ActivityLog.create({ userId: req.auth.userId, boardId: task.boardId, taskId: task._id, action: 'Set Reminder' });
...
router.post('/boards', async (req, res) => {
  const { title, owner, organizationId, imageUrl } = req.body;
  const board = new Board({ title, owner, organizationId, imageUrl, members: [owner] });
  await board.save();
  res.json(board);
});
...
router.get('/organizations', async (req, res) => {
  try {
    const userOrganizations = await organizations.getOrganizationMembershipList({ userId: req.auth.userId });
