...
router.post('/tasks/:taskId/reminder',  requireAuth(), async (req, res) => {
  const { userId } = getAuth(req);
  const task = await Task.findById(req.params.taskId);
  if (!task) return res.status(404).json({ error: 'Task not found' });
  task.reminder = true;
...
router.get('/boards/:organizationId', requireAuth(), async (req, res) => {
...
router.get('/activity/organization/:organizationId',  async (req, res) => {
  ...
  router.get('/activity/board/:boardId', requireAuth(),async (req, res) => {
    const logs = await ActivityLog.find({ boardId: req.params.boardId }).sort({ timestamp: -1 });
    res.json(logs);
  });
  
  // Fetch activity logs for a specific task
  router.get('/activity/task/:taskId', requireAuth(), async (req, res) => {
  ...
  router.get('/activity/organization/:organizationId/chart', requireAuth(),async (req, res) => {
